...
class Rule_AST_Transformer(ast.NodeTransformer):
    ...
    def visit_Tuple(self, node):
        ...
        if not (
            ...
            or isinstance(count, ast.Constant) and isinstance(count.value, (int, float, complex))
        ):
                ...
                value=ast.Attribute(
                    ...
                    value=ast.Subscript(
                        value=ast.Attribute(
                            ...
                            attr='worlds',
                            ctx=ast.Load()),
                        slice=ast.Constant(self.player),
                        ctx=ast.Load()),
                    attr=node.value.id,
                    ctx=ast.Load()),
                slice=ast.Constant(s.id.replace('_', ' ')),
                ctx=node.ctx)
        else:
            return node
        ...
        if items:
            node.values = [ast.Call(
                func=ast.Attribute(
                    ...
                    ctx=ast.Load()),
                args=[ast.Tuple(elts=[ast.Constant(i) for i in items], ctx=ast.Load()), ast.Constant(self.player)],
                keywords=[])] + new_values
        else:
    ...
    def replace_subrule(self, target, node):
        ...
        item_rule = ast.Call(
            func=ast.Attribute(
                ...
                attr='has',
                ctx=ast.Load()),
            args=[ast.Constant(subrule_name), ast.Constant(self.player)],
            keywords=[])
